...
function setTab(c){
	...
	$.ajax({
		type:"post",
		url:"/weixinTextBook.app?method=getMyTextBookOrderByAjax",
		data:"pagenum=0&userid=${userid}&type="+c,
		success:function(data){
			if(data!=""){
				$("#con").html(data); 
			}else{
				$(document).unbind('scroll');
				$("#con").html("<div style='width:100%;background-color:#fff;text-align:center;height:85px;line-height:85px;'>暂无数据</div>"); 
			}
			num++;
		}
	});
...
<div class="Menubox">
    <ul>
        ...
        <li id="one1" onclick="setTab(1)" style="width:30%;" class="hover">已发货</li>
        <li id="one2" onclick="setTab(2)" style="width:30%;" >未发货</li>
    </ul>
</div>
