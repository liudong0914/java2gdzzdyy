    ...
    <div class="container_answer">
    	<div class="container_answer_size">
        	...
        	<div class="container_answer_size_radio" onclick="returnAnswer('<%=model.getOptionNo(i) %>')">
        		...
        		<label id="option_<%=model.getOptionNo(i) %>" <%if(answer != null && model.getOptionNo(i).equals(answer.getAnswer())){ %>style="color:<%=answer_color %>"<%} %>><%=model.getOptionNo(i) %>、<%=model.getOptionValue(i) %><a id="option_<%=model.getOptionNo(i) %>_style"><%if(answer != null && model.getOptionNo(i).equals(answer.getAnswer())){ %><%if(model.getRightans().equals(answer.getAnswer())){ %><span style="float:right; text-align:right; color:#009900; font-size:20px;">√</span><%}else{%><span style="float:right; text-align:right; color:#FF0000; font-size:20px;">×</span><%}} %></a></label><br/>
            </div>
            <%} %>
        </div>
    </div>
