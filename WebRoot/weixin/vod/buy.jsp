...
<%@page import="com.wkmk.tk.bo.TkBookContentPrice"%>
...
<div class="payment_time_mask">
    <div class="wk_buy" >
        ...
        <p class="wk_buy_p2">龙门作业宝</p> 
        <p class="wk_buy_p3">¥${contentPrice.sellprice }<span style="font-size:12px;">(学币)</span></p> 	
        ...
        TkBookContentPrice price = (TkBookContentPrice)request.getAttribute("contentPrice");
        SysUserInfo sysUserInfo = (SysUserInfo)request.getAttribute("sysUserInfo");
        if(sysUserInfo.getMoney() < price.getSellprice() || "0".equals(sysUserInfo.getPaypassword())){
        	if(sysUserInfo.getMoney() < price.getSellprice()){
        		iswxpay = "1";//微信直接在线购买
        %>
    ...
    <div class="pay" <%if(sysUserInfo.getMoney() < price.getSellprice() || "0".equals(sysUserInfo.getPaypassword()) || "0".equals(paypassword)){ %>style="display:none;"<%} %>>
...
<div class="search" style="z-index:999999;">
    ...
    <div class="search_wk">
		...
		<a class="search_wk_a" style="margin-left:50px;">解题微课</a>
		<%}else if("2".equals(tkBookInfo.getVodstate())){ %>
		<a class="search_wk_a" style="margin-left:50px;">举一反三</a>
		<%}else if("3".equals(tkBookInfo.getVodstate())){ %>
		<a href="/weixinVod.app?method=buy&userid=${userid }&bookid=${bookid }&bookcontentid=${bookcontentid }&searchtype=${searchtype }&buytype=1" <%if("1".equals(buytype)){ %>class="search_wk_a"<%} %>>解题微课</a>
		<a href="/weixinVod.app?method=buy&userid=${userid }&bookid=${bookid }&bookcontentid=${bookcontentid }&searchtype=${searchtype }&buytype=2" <%if("2".equals(buytype)){ %>class="search_wk_a"<%} %>>举一反三</a>
		<%} %>
    </div>
...
<div class="wk">
	<img src="/upload/${tkBookInfo.sketch }" class="wk_img"/>
	<p class="wk_p">${tkBookInfo.title }(${tkBookInfo.flags })／${parent.title }／${tkBookContent.title }</p>
    <div  class="wk_money">
    	<%if(price.getSellprice() > 0){ %>
    	<p class="wk_money_p01">¥</p>
        <p class="wk_money_p">${contentPrice.sellprice }<span style="font-size:12px;">(学币)</span></p>
        <%}else{ %>
        <p class="wk_money_p">免费<!-- <span style="font-size:12px;color:#11aa11;">(点击底部的“立即购买”可直接观看)</span> --></p>
        <%} %>
    </div>
    <div class="wk_font">
    	<span>${contentPrice.discount}折</span>
        <span class="wk_font_span" >定价： <strike>${contentPrice.price }(学币)</strike></span>
    	<span class="wk_font_span">销量：${contentPrice.sellcount }笔</span>
    </div>
</div>
...
<div id="Tab1">
    <div class="Menubox">
    <ul>
    	<li class="Menubox_li"></li>
        <li id="one1" onclick="setTab('one',1,2)" class="hover">详情</li>
        <li id="one2" onclick="setTab('one',2,2)" >评价</li>
    </ul>
    </div>
    <div class="Contentbox">
        <div id="con_one_1" class="hover">
        	<p class="Contentbox_p"><%if("1".equals(buytype)){ %>[解题微课]<%}else{ %>[举一反三微课]<%} %></p>
        	<%List filmList = (List)request.getAttribute("filmList"); %>
            <p class="Contentbox_p02">共<span><%=filmList.size() %></span>个微课</p>
            <%
            TkBookContentFilm film = null;
            for(int i=0, size=filmList.size(); i<size; i++){
            	film = (TkBookContentFilm)filmList.get(i);
            %>
            <p><%=i+1 %>.<%=film.getTitle() %></p>
            <%} %>
        </div>
...
<%}else{ %>
<div class="payment_time_title">   
	<a class="button" style="background:#3cd5a2;">立即查看</a>
</div>
<%} %>
...
function pay(){
    ...
    if(check == "1"){
    	$.ajax({
	        type:"post",
	        url:"/weixinVod.app?method=pay",
	        data:"userid=${userid}&priceid=${contentPrice.priceid}&paypwd=" + paypwd,
	        success:function(msg){
	        	if(msg == "ok"){
	        		window.location.replace("/weixinVod.app?method=play&userid=${userid }&bookid=${bookid }&bookcontentid=${bookcontentid }&type=${buytype }&searchtype=${searchtype }");
	        	}else{
...
function getPwd(){
	...
	var redirecturl = "%2fweixinVod.app%3fmethod%3dbuy%26userid%3d${userid }%26bookid%3d${bookid }%26bookcontentid%3d${bookcontentid }%26searchtype%3d${searchtype }";//转码处理
...
$(function () {
    $(".payment_time_title a").click(function () {
    	...
    	$.ajax({
	        type:"post",
	        url:"/weixinVod.app?method=pay",
	        data:"userid=${userid}&priceid=${contentPrice.priceid}",
	        success:function(msg){
	        	if(msg == "ok"){
	        		window.location.replace("/weixinVod.app?method=play&userid=${userid }&bookid=${bookid }&bookcontentid=${bookcontentid }&type=${buytype }&searchtype=${searchtype }");
	        	}
...
function wxpay(){
    ...
    var buytitle = "购买龙门作业宝作业习题解题微课";
    <%if(!"1".equals(buytype)){ %>
    	buytitle = "购买龙门作业宝作业习题举一反三微课";
    <%} %>
   	wx.checkJsApi({
   	    	...
   	    	if(res.checkResult.chooseWXPay == true){
   	    	    ...
   	    	    $.ajax({
   	    	    	type:"post",
   	    	       	...
   	    	       	data:"tradetype=2&money=${contentPrice.sellprice }&priceid=${contentPrice.priceid }&openid=${openid}&userid=${userid}&body=" + buytitle + "&timestamp=<%=jsticket.get("timestamp") %>&nonce_str=<%=jsticket.get("nonceStr") %>",
   	    	       	async: false,
   	    	       	success:function(msg){
   	    	       		//ajax返回消息后，可以去掉蒙板
   	    	       		if(msg != "0"){
   	    	       			...
   	    	       			wx.chooseWXPay({
   	    	           		    ...
   	    	           		    success: function (res) {
   	    	           		        ...
   	    	           		        var redirecturl = "%2fweixinVod.app%3fmethod%3dplay%26userid%3d${userid }%26bookid%3d${bookid }%26bookcontentid%3d${bookcontentid }%26type%3d${buytype }%26searchtype%3d${searchtype }";//转码处理
