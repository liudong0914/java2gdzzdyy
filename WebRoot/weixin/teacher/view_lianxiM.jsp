...
<div class="search search_write">
	<a href="/weixinTeacher.app?method=detailErrorQuestion&userid=${userid }&classid=${classid }&bookcontentid=${bookcontentid }"><div  class="search_left">
    	<img src="/weixin/images/a01.png" class="search_class" />
    </div></a>
    <div  class="search_wk">
    	<p class="search_wk_p">查看试题</p>
    </div>
...
<div id="container">
    		...
    		<div class="container_answer_size">
	        	...
	        	<div class="container_answer_size_radio">
	        		<input type="radio" name="itemA" value="<%=tqi.getOptionNo(i) %>"/>
	        		  <lable><%=tqi.getOptionNo(i) %>、<%=tqi.getOptionValue(i) %></lable>
	            </div>
	            <%} %>
        	</div>
    	</div>
    ...
    <div class="conrainer_button">
    	<%if(similarCounts.intValue() > 0){ %><a href="/weixinStudent.app?method=viewSimilar&userid=${userid }&bookcontentid=${bookcontentid }&bookid=${bookid }&classid=${classid }&questionid=${model.questionid }&type=2&qid=${model.questionid }&vtype=8" class="conrainer_button1">举一反三</a><%} %>
        <%if(filmCounts.intValue() > 0){ %><a href="/weixinStudent.app?method=videoPlay&userid=${userid }&bookcontentid=${bookcontentid }&bookid=${bookid }&classid=${classid }&questionid=${model.questionid }&type=2&qid=${model.questionid }&vtype=8" class="conrainer_button1">看微课</a><%} %>
    </div>
    <%} %>
