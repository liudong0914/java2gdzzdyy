...
<div class="box_tool_min padding_top2 padding_bottom2 padding_right0">
	...
	<div class="right">
		...
		<a href="javascript:;" onclick="batchRecord('/vwhFilmInfoCheckAction.do?method=checkBatchRecord', '您确定审核通过选中微课?')"><span class="icon_reply">审核</span></a>
		<div class="box_tool_line"></div>
		<a href="javascript:;" onclick="batchRecord('/vwhFilmInfoCheckAction.do?method=delBatchRecord', '您确定删除选中微课并存入回收站中?')"><span class="icon_recycle">删除</span></a>
		<div class="clear"></div>
		</div>
	</div>		
	</div>	
...
<div id="scrollContent" >
	<table class="tableStyle" useClick="false" useCheckBox="true" sortMode="true">
		...
		<tr>
			...
			<td class="ali01"><span class="text_slice" style="width:350px;" title="<%=vfi.getTitle() %>"><%=vfi.getTitle() %></span></td>
			<td class="ali02"><%=vfi.getActor() %></td>
			<td class="ali02"><%=vfi.getSysUserInfo().getUsername() %></td>
			<td class="ali02"><%=vfi.getCreatedate() %></td>
			<td class="ali02">
			<div class="img_edit hand" title="修改" onclick="editThisRecord('vwhFilmInfoCheckAction.do','<%=vfi.getFilmid() %>')"></div>
...
<script type="text/javascript">
	...
	//提交表单
	function postData(){
	    document.pageForm.action = '/vwhFilmInfoCheckAction.do?method=list';
	...
	function addFilmPix(filmid){
		...
		diag.Title = "微课视频";
		diag.URL = '/vwhFilmPixAction.do?method=list&filmid='+filmid;
		diag.Width = 800;
		diag.Height = 600;
