...
<html:form action="/vwhFilmInfoRecyclebinAction.do" method="post">
...
function saveRecord(){
	if(validateForm('form[name=vwhFilmInfoActionForm]')){
		...
		keditor.sync();
	  	objectForm.action="vwhFilmInfoRecyclebinAction.do?method=<bean:write name="act"/>";
	  	objectForm.submit();
	}
}

function backRecord(){
  	document.vwhFilmInfoActionForm.action="/vwhFilmInfoRecyclebinAction.do?method=list";
...
function uploadPhoto(){
	...
	diag.Title = "上传图片";
	diag.URL = '/sysImageUploadAction.do?method=uploadimage&savepath=vwh&pathtype=ID&sketch=false&swidth=500&sheight=333';
	diag.Width = 350;
	diag.Height = 180;
