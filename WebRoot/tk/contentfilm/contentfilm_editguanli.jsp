	...
	<table class="tableStyle" formMode="line" align="center">
		...
		<tr>
			...
			<td colspan="3">
				...
				<!--因为编辑改为弹出窗口  --><button type="button" onclick="javascript:history.back();"><span class="icon_back">返回</span></button>
			</td>
		</tr>
	</table>
...
function saveRecord(){
	if(validateForm('form[name=tkBookContentFilmActionForm]')){
		...
		var bookcontentid = document.getElementById("bookcontentid").value;
	  	objectForm.action="tkBookContentFilmAction.do?method=<bean:write name="act"/>&bookcontentid="+bookcontentid+"&status=1";
	  	objectForm.submit();
	}
}
...
function backRecord(){
	var bookcontentid = document.getElementById("bookcontentid").value;
  	document.tkBookContentFilmActionForm.action="/tkBookContentFilmAction.do?method=listguanli&bookcontentid="+bookcontentid+"&status=1";
