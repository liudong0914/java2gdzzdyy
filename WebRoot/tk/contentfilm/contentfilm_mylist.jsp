...
//提交表单
function postData(){
    document.pageForm.action = '/tkBookContentFilmAction.do?method=mycontentfilmlist';
		document.pageForm.submit();
}
function editFilm(obj){
	document.pageForm.action = '/tkBookContentFilmAction.do?method=beforeUpdatemy&objid='+obj;
	document.pageForm.submit();
}
function addFilmPix(filmid){
  ...
  <body>
    ...
    <div class="box3" panelTitle="我的微课" style="margin-right:0px;padding-right:0px;">
     <table>
       <tr>
         <td>微课名称：</td>
		...
		<select name="status" id="status">    
			<option value="" <logic:equal name="status" value="">selected</logic:equal>>全部</option>
			<option value="0" <logic:equal name="status" value="0">selected</logic:equal>>待审核</option>
			<option value="1" <logic:equal name="status" value="1">selected</logic:equal>>审核通过</option>
			</select>
		</td>
...
<!--<div class="box_tool_min padding_top2 padding_bottom2 padding_right0">
	...
	<div class="right">
		...
		<a href="javascript:;" onclick="batchRecord('/tkBookContentFilmAction.do?method=delBatchRecordmy', '您确定删除选中微课?')"><span class="icon_recycle">删除</span></a>
		<span style="float:right;color:green;">说明：‘视频转码’为未完成时，如果此时审核微课，未完成转码的视频将无法观看。</span>
		<div class="clear"></div>
		</div>
	</div>		
	</div>	
	...
	<table class="tableStyle" useClick="false" useCheckBox="true" sortMode="true">
		<tr>
			...
			<th class="ali02">微课类型</th>
			<th class="ali02">微课状态</th>
			<th class="ali02" width="50">序号</th>
			<th class="ali02" width="50">作者</th>
		...
		<tr>
			...
			<td class="ali02">
				<div class="img_edit hand" title="修改" onclick="editFilm('${model.fid}')"></div>
				<div class="img_cd_driver hand" title="微课视频" onclick="addFilmPix('${model.filmid}')"></div>
			</td>
		</tr>
		</c:forEach>
	</table>
