...
<script type="text/javascript">
	...
	//提交表单
	function postData(){
	    document.pageForm.action = '/sysAdminUserRoleAction.do?method=outroleuser';
	...
	function doOnLoad(){
	 	window.parent.frames["right"].location = "/sysAdminUserRoleAction.do?method=inroleuser&roleid=<bean:write name="roleid"/>";
	}
	  ...
	  if(num>0){
		...
		if (checkids=="") {
		 ...
		 top.Dialog.confirm(str,
			   function(){
	               document.pageForm.action="/sysAdminUserRoleAction.do?method=addroleuser";
		   		   document.pageForm.submit(); 
		       }
		   );
		}
	...
	function addUserAll(){
	  if(num>0){
		  ...
		  top.Dialog.confirm(str,
			   function(){
	               document.pageForm.action="/sysAdminUserRoleAction.do?method=addroleuserall";
		   		   document.pageForm.submit(); 
		       }
		   );
	  }
