...
<div class="box3" panelTitle="部门成员" style="margin-right:0px;padding-right:0px;">
	<table>
		<tr>
			<td>真实姓名：</td>
...
<div id="scrollContent" >
	<table class="tableStyle" useClick="false" useCheckBox="true" sortMode="true">
		...
		<tr>
			<td><input type="checkbox" name="checkid" value="<bean:write property="memberid" name="model"/>"/></td>
...
<script type="text/javascript">
	...
	//提交表单
	function postData(){
	    document.pageForm.action = '/sysUnitDeptMemberAction.do?method=inmember';
	...
	function doOnLoad(){
	 	window.parent.frames["left"].location = "/sysUnitDeptMemberAction.do?method=outmember&deptid=<bean:write name="deptid"/>"; 
	}
	
	function delUser(){
	  if(num>0){
		  var str="此操作将给选中成员从当前部门中删除，真的要继续么？";
		  var checkids="";
		  if (num>1){
		  for(i=0;i<num;i++){
		...
		if (checkids=="") {
		     top.Dialog.alert("您还没有选择待删除成员呢!")
		}
		else{
		   top.Dialog.confirm(str,
			   function(){
	               document.pageForm.action="/sysUnitDeptMemberAction.do?method=deletemember";
		   		   document.pageForm.submit(); 
		       }
		   );
		}
	...
	function delUserAll(){
	  if(num>0){
		  ...
		  top.Dialog.confirm(str,
			   function(){
	               document.pageForm.action="/sysUnitDeptMemberAction.do?method=deletememberall";
		   		   document.pageForm.submit(); 
		       }
		   );
	  }
