...
<html:form action="/sysUmsUnitInfoAction.do" method="post">
	<table class="tableStyle" formMode="line" align="center">
		<tr>
			<th colspan="2"><logic:equal value="addSave" name="act">新增单位</logic:equal><logic:equal value="updateSave" name="act">修改单位</logic:equal></th>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="ali01">
				<java2code:option name="sysUnitInfo.schooltype" codetype="schooltype" property="schooltype" ckname="1" onchange="changeSchoolType(this.value)"/>
				<input type="hidden" name="stype" id="stype" value="<bean:write property="schooltype" name="model"/>"/>
			</td>
		</tr>
		<tr id="schooltype_1" <%if(!"1".equals(model.getSchooltype())){ %>style="display:none;"<%} %>>
			<td class="ali03">学校性质：</td>
			<td class="ali01">
				<input type="checkbox" name="schooltype1" value="1" id="schooltype1_1" <%if(model.getType().indexOf("1") != -1){ %>checked="checked"<%} %>/>幼教&nbsp;&nbsp;&nbsp;
              	<input type="checkbox" name="schooltype1" value="2" id="schooltype1_2" <%if(model.getType().indexOf("2") != -1){ %>checked="checked"<%} %>/>小学&nbsp;&nbsp;&nbsp;
				<input type="checkbox" name="schooltype1" value="3" id="schooltype1_3" <%if(model.getType().indexOf("3") != -1){ %>checked="checked"<%} %>/>初中&nbsp;&nbsp;&nbsp;
				<input type="checkbox" name="schooltype1" value="4" id="schooltype1_4" <%if(model.getType().indexOf("4") != -1){ %>checked="checked"<%} %>/>高中
			</td>
		</tr>
		<tr id="schooltype_2" <%if(!"2".equals(model.getSchooltype())){ %>style="display:none;"<%} %>>
			<td class="ali03">学校性质：</td>
			<td class="ali01">
				<input type="checkbox" name="schooltype2" value="1" id="schooltype2_1" <%if(model.getType().indexOf("1") != -1){ %>checked="checked"<%} %>/>中职&nbsp;&nbsp;&nbsp;
              	<input type="checkbox" name="schooltype2" value="2" id="schooltype2_2" <%if(model.getType().indexOf("2") != -1){ %>checked="checked"<%} %>/>高职&nbsp;&nbsp;&nbsp;
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="ali01"><input type="text" style="width:60px;" class="validate[required,custom[onlyNumber],length[0,4]]" name="sysUnitInfo.orderindex" value="<bean:write property="orderindex" name="model"/>"/><span class="star">*</span></td>
		</tr>
		<tr>
			<td class="ali03">是否推荐：</td>
			<td class="ali01"><java2code:option name="sysUnitInfo.recommand" codetype="boolean" property="recommand" ckname="1"/></td>
		</tr>
		<tr>
			<td class="ali03">推荐编号：</td>
			<td class="ali01"><input type="text" style="width:60px;" class="validate[required,custom[onlyNumber],length[0,4]]" name="sysUnitInfo.recommandno" value="<bean:write property="recommandno" name="model"/>"/><span class="star">*</span></td>
		</tr>
		<tr>
			<td class="ali03">状态：</td>
			<td class="ali01"><java2code:option name="sysUnitInfo.status" codetype="status4" property="status" ckname="1"/></td>
		</tr>
		<tr>
...
<input type="hidden" name="schooltype" value="<bean:write name="schooltype"/>"/>
<input type="hidden" name="province" value="<bean:write name="province"/>"/>
<input type="hidden" name="city" value="<bean:write name="city"/>"/>
<input type="hidden" name="county" value="<bean:write name="county"/>"/>
<!-- 分页与排序 -->
...
function saveRecord(){
	if(validateForm('form[name=sysUnitInfoActionForm]')){
		...
		if(stype == '1'){
			if(!(document.getElementById('schooltype1_1').checked || document.getElementById('schooltype1_2').checked || document.getElementById('schooltype1_3').checked || document.getElementById('schooltype1_4').checked)){
				top.Dialog.alert("请选择学校性质!");
				return false;
			}
		}
		if(stype == '2'){
			if(!(document.getElementById('schooltype2_1').checked || document.getElementById('schooltype2_2').checked)){
				top.Dialog.alert("请选择学校性质!");
				return false;
			}
		}
		...
		var objectForm = document.sysUnitInfoActionForm;
	  	objectForm.action="sysUmsUnitInfoAction.do?method=<bean:write name="act"/>";
	  	objectForm.submit();
	}
}

function backRecord(){
  	document.sysUnitInfoActionForm.action="/sysUmsUnitInfoAction.do?method=list";
  	document.sysUnitInfoActionForm.submit();
}
...
function changeSchoolType(schooltype){
	document.getElementById("stype").value = schooltype;
	if(schooltype == '1'){
		document.getElementById("schooltype_1").style.display = "";
		document.getElementById("schooltype_2").style.display = "none";
	}else if(schooltype == '2'){
		document.getElementById("schooltype_1").style.display = "none";
		document.getElementById("schooltype_2").style.display = "";
	}else{
		document.getElementById("schooltype_1").style.display = "none";
		document.getElementById("schooltype_2").style.display = "none";
	}
