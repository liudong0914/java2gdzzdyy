...
<html:form action="/sysUserInfoAction.do" method="post">
	<table class="tableStyle" formMode="line" align="center">
		...
		<tr>
			<td class="ali03">是否认证：</td>
			<td class="ali01"><java2code:option  name="sysUserInfo.authentication" codetype="authentication" property="authentication" ckname="1"/></td>
		</tr>
		<tr>
			<td class="ali03">出生日期：</td>
			<td class="ali01">
				<input type="text" name="sysUserInfoDetail.birthday" readonly="readonly" value="<bean:write property="birthday"  name="detail"/>" class="date validate[required,custom[date]]"/>(日期格式：2008-08-08)<span class="star">*</span>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="ali01">
				...
				<select name="sysUserInfoDetail.county" id="selectCounty" boxHeight="130" onchange="changeArea1(this.value)">
					<option value="">请选择...</option>
					<%
		...
		<logic:equal value="addSave" name="act">
		<tr>
			<td class="ali03">所属学校：</td>
			<td class="ali01">
			<select name="sysUserInfo.unitid" id="selectUnit" boxHeight="130">
				<option value="">请选择...</option>
			</select>
			</td>
		</tr>
		...
		<tr>
			<td class="ali03">所属学校：</td>
			<td class="ali01"><bean:write property="unitname" name="unitInfo"/>
...
<input type="hidden" name="xueduan" value='<bean:write name="xueduan"/>'/>
<input type="hidden" name="authentication" value='<bean:write name="authentication"/>'/>
...
function saveRecord(){
	if(validateForm('form[name=sysUserInfoActionForm]')){
		...
		var objectForm = document.sysUserInfoActionForm;
	  	objectForm.action="sysUmsUserInfoAction.do?method=<bean:write name="act"/>";
	  	objectForm.submit();
	}
}
...
function backRecord(){
  	document.sysUserInfoActionForm.action="/sysUmsUserInfoAction.do?method=list";
...
function changeArea1(citycode){
	<logic:equal value="addSave" name="act">
		if(citycode != ""){
			$.ajax({
		        type: "get",
		        async: false,
		        url: "sysUmsUserInfoAction.do?method=getUnitList&tag=3&citycode=" + citycode + "&type=1&ram=" + Math.random(),
		        dataType: "text",
		        success: function(data){
