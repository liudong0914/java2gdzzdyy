...
var uploadFileList = "";//全局变量，控制重复上传
function uploadSuccess(file, serverData) {
	try {
		...
		if(serverData.indexOf("exist_") != -1){
			document.getElementById(fileId).cells[0].innerHTML = document.getElementById(fileId).cells[0].innerHTML + "&nbsp;<font color='red'>(此视频已经上传过!)</font>";
		}else if(serverData.indexOf("error_") != -1){
			document.getElementById(fileId).cells[0].innerHTML = document.getElementById(fileId).cells[0].innerHTML + "&nbsp;<font color='red'>(上传错误!)</font>";
		}else{
			var keystr = "[" + file.name + "-" + file.size + "]";
			if(uploadFileList.indexOf(keystr) != -1){
				document.getElementById(fileId+"_filepath").value = "exist_" + serverData;
				document.getElementById(fileId).cells[0].innerHTML = document.getElementById(fileId).cells[0].innerHTML + "&nbsp;<font color='red'>(此视频已经上传过!)</font>";
			}else{
				uploadFileList += keystr;
			}
		}
	} catch (ex) {
		this.debug(ex);
...
function addReadyFileInfo(file){
	...
	var imgurl = "/swfupload/images/btn_01.jpg";
...
function deleteFile(fileId){
	...
	if(infoTable.rows.length == 1){
		...
		var imgurl = "/swfupload/images/btn_02.jpg";
		swfu.settings.button_image_url = imgurl;
		swfu.callFlash("SetButtonImageURL", [imgurl]);
