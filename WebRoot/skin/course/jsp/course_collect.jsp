...
<%@page import="com.wkmk.edu.bo.EduCourseCollect"%>
<%@page import="com.wkmk.util.common.Constants"%>
<%@ include file="/libs/jsp/taglibs.jsp"%>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><%=Constants.PRODUCT_NAME %>-课程收藏</title>
...
function postData(){
    document.pageForm.action = "/courseCenter.do?method=courseCollect&mark=${mark }";
		...
		<div class="mainBox">
			...
			<div class="mainCon mainCon01">
				<div class="tab">
					...
					<div class="acts">
						<input type="button" class="btn btn-pop ml20" value="删除" onclick="delRecord('/courseCenter.do?method=delBatchCollect')">
					</div>
				</div>
				...
				<table width="100%" class="table">
					...
					<tbody>
						...
						if(list != null && list.size() > 0){
							...
							EduCourseCollect ecc = null;
							for(int i=0, size=list.size(); i<size; i++){
								ecc = (EduCourseCollect)list.get(i);
								eci = ecc.getEduCourseInfo();
						%>
						<tr>
							<td colspan="7">
							<div class="tableBox box02">
								<table width="100%" style="border-collapse:separate;">
									...
									<tr>
										<th width="45">
											<input class="checkShow msgCheck" type="checkbox" name="checkid" value="<%=ecc.getCollectid() %>"/>
										</th>
										<th style="text-align:left;"><%=eci.getTitle() %></th>
										<th width="80"><%=eci.getClasshour() %></th>
										<th width="80"><%=eci.getCollects() %></th>
										<%if(eci.getPrice().floatValue() > 0){ %>
										<th width="60"><%=eci.getPrice() %></th>
										<%}else{ %>
										<th width="60" style="color:#56bc78;">免费</th>
										<%} %>
										<th width="140"><%=ecc.getCreatedate() %></th>
										<th width="60">
											<a class="alink" href="/course/<%=eci.getCourseid() %>.htm" target="_blank">查看</a>
										</th>
									</tr>
									</thead>
								</table>
