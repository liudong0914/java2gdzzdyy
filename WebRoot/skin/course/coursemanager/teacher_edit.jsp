...
<div class="mainCon-bd">
	<div class="form-body creatCourseStep">
		<div class="form-row form-row01">
			...
			<div class="input">
				<input keepDefaultStyle="true" class="validate[required] ipt-text formEleToVali" id="sysUserInfo.loginname" name="sysUserInfo.loginname" value="${model.loginname }" style="width:200px;" type="text" onblur="checkCode(this.value, '1')"><span id="div_loginname" style="color:red;display:none;"></span></td>
			</div>
		</div>
		
		<span id="editUser" style="color:red;display:none;">
		<div class="form-row">
			<div class="label">
				<label>
		...
		<span id="editUser2" style="display:;">
		<div class="form-row">
			<div class="label">
				<label>
				<i class="text-red must">*</i>
		...
		<div class="form-row">
			...
			<div class="input">
				<input type="checkbox" style="margin-right: 2px;" name="moduleid" value="1" />课程信息&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" style="margin-right: 2px;" name="moduletype0" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype0" value="2" />管理</br>
                
                <input type="checkbox" style="margin-right: 2px;" name="moduleid" value="2" />课程目录&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" style="margin-right: 2px;" name="moduletype1" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype1" value="2" />管理</br>
                                
                <input type="checkbox" style="margin-right: 2px;" name="moduleid" value="3" />微课管理&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" style="margin-right: 2px;" name="moduletype2" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype2" value="2" />管理</br>
                                
                <input type="checkbox" style="margin-right: 2px;" name="moduleid" value="4" />课程资源&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" style="margin-right: 2px;" name="moduletype3" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype3" value="2" />管理</br>
                               
                <input type="checkbox" style="margin-right: 2px;" name="moduleid" value="6" />学员管理&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="radio" style="margin-right: 2px;" name="moduletype4" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype4" value="2" />管理</br>
                <!--                
                <input type="checkbox" style="margin-right: 2px;" name="moduleid" value="7" />课程答疑&nbsp;&nbsp;&nbsp;&nbsp;
                <input type="radio" style="margin-right: 2px;" name="moduletype5" value="1" checked="checked"/>查看
                <input type="radio" style="margin-right: 2px;" name="moduletype5" value="2" />管理</br>
                 -->
			</div>
		</div>
...
<script>
function saveRecord(){
	var flag = document.getElementById("editUser").style.display;
	if(flag == "none"){
		var loginname = document.getElementById("sysUserInfo.loginname").value;
...
function checkCode(value,checktype){
  	if(value != ''){
	  	var url = 'sysUserInfoAction.do?method=checkName2&checkname='+value+'&courseid='+${courseid}+'&courseclassid='+${courseclassid}+'&checktype='+checktype+'&ram=' + Math.random();
	  	$.ajax({
		  type: 'post',
		  url: url,
		  async: false,//同步请求
		  ...
		  success: function(data){
		  	if(data == '0'){
		  		...
		  		//该用户存在，不为教师类型
		  		document.getElementById("div_loginname").style.display = '';
		  		document.getElementById("div_loginname").innerText="该用户类型不为教师，无法成为助教！";
		  		document.getElementById("editUser").style.display = 'none';
		  		document.getElementById("editUser2").style.display = 'none';
		  		document.getElementById("userid").value = "";
			  	document.getElementById("btn_save").style.display = 'none';
			  	...
			  	//该用户存在，未加入
		  	 	document.getElementById("div_loginname").style.display = '';
		  	 	document.getElementById("div_loginname").innerText="该用户已存在，请直接授权！";
		  	 	document.getElementById("editUser").style.display = 'none';
		  	 	document.getElementById("editUser2").style.display = '';
